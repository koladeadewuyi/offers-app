  ...
  describe("OfferValidator") {
    describe("apply") {
      it("should return an empty list when given a valid offer") {
        val validOffer = Offer(randomUUID, ValidDescription, Price(ValidCost), Validity(now, nextDay))
      ...
      val errorScenarios = Table(
        ...
        ("cost is not > 0", randomUUID, ValidDescription, CostZeroOrLess, now, nextDay, Seq(("cost", "offer cost must be greater than 0"))),
        ...
        ("multiple invalid fields i.e. offerId is not > 10 chars and cost is not > 0", InvalidOfferId, ValidDescription, CostZeroOrLess, now, nextDay, Seq(("offerId", "offerId must exceed 10 characters"), ("cost", "offer cost must be greater than 0")))
      )
      ...
      forAll(errorScenarios) { (testDescription, offerId, description, cost, startDate, endDate, invalidFieldToErrorMsgs) =>
        it(s"should return a list of corresponding error info when $testDescription") {
          withSystemTimeSetTo(now) {
            val validOffer = Offer(offerId, description, Price(cost), Validity(startDate, endDate))
