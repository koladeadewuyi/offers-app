...
class ServerSpec extends TestFixture with MarshallingTestUtils with TryValues with AppConfig with GivenWhenThen {
  ...
  describe("Server") {
    it("should allow creation of an offer") {
      val entity = createOfferBody(ValidDescription, ValidCost, now, nextDay)
    ...
    it("should reject creation of an invalid offer") {
      val invalidOfferEntity = createOfferBody(ValidDescription, CostZeroOrLess, now, twoDaysAgo)
      val expectedValidationErrors = ValidationRejection(Seq(
        FieldErrorInfo("cost", "offer cost must be greater than 0"),
    ...
    it("should allow update of an offer") {
      val entity = createOfferBody(ValidDescription, ValidCost, now, nextDay)
      val entityUpdate = createOfferBody(ValidDescription, ValidCost, nextThreeHours, nextDay)
    ...
    it("should allow retrieval of all offers") {
      val firstEntity = createOfferBody(ValidDescription, ValidCost, now, nextDay)
      val secondEntity = createOfferBody(ValidDescription, ValidCost, nextThreeHours, nextDay)
  ...
  private def makeRequest(url: String, method: HttpMethod = GET, entity: String = "")
                         (implicit actorSystem: ActorSystem, materializer: ActorMaterializer) = {
    Http().singleRequest(
      HttpRequest(method, Uri(s"http://$ApiHost:$ApiPort$url"),
