  ...
  describe("DefaultOfferRepository") {
    ...
    describe("get") {
      it("should retrieve an existing offer when given the offerId") {
        val offer = Offer(randomUUID, ValidDescription, price(ValidAmount), Validity(now, nextDay), Option(Expired.No))
        val dataStoreMock = createDataStore()
    ...
    describe("update") {
      it("should update an existing offer when given an update") {
        val originalOffer = Offer(randomUUID, ValidDescription, price(ValidAmount), Validity(now, nextDay))
        val updatedOffer = Offer(originalOffer.offerId, ValidDescription, price(ValidAmount), Validity(now, nextDay))
        val dataStoreMock = createDataStore()
      ...
      it("should return a failed future with appropriate message when the dataStore throws an exception") {
        val updatedOffer = Offer(randomUUID, ValidDescription, price(ValidAmount), Validity(now, nextDay))
    ...
    describe("retrieveAll") {
      it("should retrieve all offers") {
        val firstOffer = Offer(randomUUID, ValidDescription, price(ValidAmount), Validity(now, nextDay), Option(Expired.No))
        val secondOffer = Offer(randomUUID, ValidDescription, price(ValidAmount), Validity(now, nextDay), Option(Expired.No))
        val dataStoreMock = createDataStore()
        dataStoreMock.put(firstOffer.offerId, firstOffer)
        dataStoreMock.put(secondOffer.offerId, secondOffer)
